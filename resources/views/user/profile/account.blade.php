...
@section('container')
<form action="{{ route('user.accountUpdate') }}" method="POST" enctype="multipart/form-data" class="container p-4 mt-5 bg-white rounded shadow">
    @csrf
    ...
    <div class="mb-3">
        ...
        <input type="email" class="form-control" id="email" name="email" value="{{ old('email', Auth::user()->email) }}" placeholder="Enter your email address" required>
        <!-- Placeholder for validation error -->
        @error('email')
            <div class="text-danger">{{ $message }}</div>
        @enderror
